...
function totalsHTML(totals, rates) {
  const rows = [
    ...
    ...(totals.taxTotal > 0 ?
      [[`Tax (${rates.taxRate}%)`, totals.taxTotal]]
    : []),
  ]
    .map(
      ([label, val]) =>
        `<tr><td class="tl">${label}</td><td>${fmt(val)}</td></tr>`,
    )
    .join("");
...
function printFooterHTML(businessInfo, customerName) {
...
${printFooterHTML(businessInfo, customerName)}
</body>
</html>`;
