...
export const DEFAULT_MARKUP_MATRIX: MarkupBracket[] = [
  { max: 1,    markupPct: 100 },
  { max: 10,   markupPct: 75  },
  { max: 25,   markupPct: 50  },
  ...
  { max: null, markupPct: 15  },
];

export function calcIncrementalMarkup(
  cost: number,
  matrix: MarkupBracket[],
  maxMarkupDollars: number | null = null,
): { markupDollars: number; effectiveRate: number } {
  const c = Number(cost);
...
export function grossProfitPct(markupPct: number): number {
